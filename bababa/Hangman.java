...
class HangmanSession {
  ...
  private void printState() {
    letterBox.print();
    System.out.println( "Hidden word : " +s);
...
class HiddenKeyword {
  ...
  public HiddenKeyword(StringBuffer gword, String message, String information) {
...
//                repaint();
  ...
  private int findOccurances( char c ) {
    int idx = rword.indexOf( c );
    pValue.setCharAt( idx, rword.charAt( idx ) );
    int counter = 1;
    while( idx != -1 ) {
      int idxx = rword.indexOf( c, idx + 1 );
      idx = idxx;
      if( idx != -1 ) {
        counter++;
        pValue.setCharAt( idx, rword.charAt( idx ) );
